        ...
        public static void Invoke(string exceptionPolicy, Action action)
        {
            ...
            while (!succeeded)
            {
                try
                {
                    ...
                    catch (UnRecoverableErrorException urex)
                    {
                        if (ExceptionPolicy.HandleException(urex, exceptionPolicy))
                        {
                            throw;
                        }
                    }
                    ...
                    {
                        Debug.WriteLine($"Fujitsu.AFC RetryableOperation.cs->Exception: {ex.Flatten()}");
