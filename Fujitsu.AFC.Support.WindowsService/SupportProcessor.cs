...
using Fujitsu.AFC.Support.WindowsService.Properties;
...
namespace Fujitsu.AFC.Support.WindowsService
{
    ...
    partial class SupportProcessor : ServiceBase, ISupportProcessor
    {
        private readonly IObjectBuilder _objectBuilder;
        private double _interval;
        private Timer _timer;

        public SupportProcessor(IObjectBuilder objectBuilder)
        {
        ...
        private void Execute(object sender, ElapsedEventArgs elapsedEventArgs)
        {
            ...
            try
            {
                var taskHandler = _objectBuilder.Resolve<ITaskHandlerManager>(TaskHandlerNames.SupportHandler);
        ...
        {
            EventLog.WriteEntry($"Support Task Processor Start Time: {Fujitsu.AFC.Tasks.Properties.Settings.Default.ServiceStartTimeHour}", EventLogEntryType.Information);
            EventLog.WriteEntry($"Support Task Processor End Time: {Fujitsu.AFC.Tasks.Properties.Settings.Default.ServiceEndTimeHour}", EventLogEntryType.Information);
            EventLog.WriteEntry($"Support Task Processor Polling Interval: {Settings.Default.PollingIntervalMinutes} minutes", EventLogEntryType.Information);
        }
    }
